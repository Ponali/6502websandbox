...
function SimulatorWidget(node) {
  ...
  function Memory() {
    var memArray = new Uint8Array(0xffff);

    function set(addr, val, assembler) {
      console.log(addr2hex(addr)+"="+val);
      let memc=getMemoryComponent(addr);
      if(memc&&(!memc.write)&&(!assembler)){
        message("tried to write in readonly area ("+addr2hex(addr)+")");
        return val;
      }
    ...
    function reset(){
      memArray=new Uint8Array(0xffff);
    }

    function get(addr) {
      console.log("read "+addr2hex(addr));
      let start=getComponentFromReference(1).start;
      if(addr==0xfffc){return start&0xff};
      if(addr==0xfffd){return start>>8};
    ...
    function storeByte(addr, value) {
      set(addr, value);
      if(currentOutput&&currentOutput.memorySet)currentOutput.memorySet(addr,value);
    }
  ...
  function Simulator() {
    ...
    function reset() {
      ...
      regA = regX = regY = 0;
      regPC = memory.getWord(0xfffc); // directly set to 0x600 before
      regSP = 0xff;
      regP = 0x30;
      updateDebugInfo();
    }
  ...
  function Assembler() {
    ...
    function defineHandle(lines){
      console.log("received ",lines)
      let out=lines;
      lines.forEach(a=>{
        a=a.split(" ").filter(a=>a!="");
        if((a[0]+"").toLowerCase()=="define"){
          let name=a[1];
          let val=a[2];
          console.log(`name=${name}, val=${val}`);
          out=out.map(a=>a.replaceAll(name,val))
        }
      });
      out=out.map(a=>a.split(" ").filter(b=>b!="")).filter(a=>(a[0]+"").toLowerCase()!="define").map(a=>a.join(" "))
      out=out.filter(a=>a.replaceAll(" ","")!="");
      console.log("returning ",out);
      return out;
    }

    function asciiValueHandle(lines){
      return lines.map(a=>a.replace(/(#".")|(#'.')/gm,(a)=>"#"+(a.match(/['"]./g)[0].charCodeAt(1))));
    }

    //let assembledCode=new Uint8Array(0xffff);

    // assembleCode()
    // "assembles" the code into memory
    function assembleCode() {
      assembledCode=new Uint8Array(0xffff);
      simulator.reset();
      labels.reset();
      defaultCodePC = getComponentFromReference(1).start; // was 0x600 before
      $node.find('.messages code').empty();

      var code = $node.find('.code').val();
      code += "\n\n";
      var lines = code.split("\n");
      lines=defineHandle(lines);
      lines=asciiValueHandle(lines);
      ...
      if (codeAssembledOK) {
        ui.assembleSuccess();
        memory.set(defaultCodePC, 0x00, true); //set a null byte at the end of the code
      } else {
    ...
    function pushByte(value) {
      memory.set(defaultCodePC, value & 0xff, true);
      defaultCodePC++;
      codeLen++;
    }
...
$(document).ready(function () {
  $('body').each(function () { // was .window#codeWindow before
